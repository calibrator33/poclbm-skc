...
group.add_option('-e', '--estimate',      dest='estimate',   default=3600,     help='estimated rate time window in seconds, default 900 (15 minutes)', type='int')
group.add_option('-t', '--tolerance',     dest='tolerance',  default=1,        help='use fallback pool only after N consecutive connection errors, default 1', type='int')
group.add_option('-b', '--failback',      dest='failback',   default=3,        help='attempt to fail back to the primary pool after N seconds, default 60', type='int')
...
group = OptionGroup(parser,
	"OpenCL Options",
	"Every option except 'platform' can be specified as a comma separated list. "
...
options.rate = max(options.rate, 10) if options.verbose else max(options.rate, 0.1)
